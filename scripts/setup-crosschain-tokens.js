...
async function proposeCrosschainToken(
    ...
    // 添加权限检查
    const hasRole = await roles.methods.hasRole(FOUNDATION_MANAGER, sender).call();
    if (!hasRole) {
    ...
    try {
        ...
        const tx = await mulSig.methods.proposeToSetCrosschainToken(
            params[0],            // token
    ...
    // 获取并签名提案
    const pendingProposals = await mulSig.methods.getPendingProposals().call({ from: sender });
    if (pendingProposals.length > 0) {
        const proposalId = pendingProposals[pendingProposals.length - 1];
        console.log('Got proposal ID:', proposalId.toString());

        // 获取所有基金会管理员
        const managers = await roles.methods.getRoleMemberArray(FOUNDATION_MANAGER).call();

        // 为每个基金会管理员签名
        for (const manager of managers) {
            const hasSigned = await mulSig.methods.hasAlreadySigned(proposalId, manager).call();
            if (!hasSigned) {
                try {
                    const gasPrice = await web3Instance.eth.getGasPrice();
                    ...
                    console.error(`Failed to get signature from manager ${manager}:`, error.message);
                }
            }
        }
        ...
        try {
            ...
            const tokenInfo = await crosschainTokens.methods.getCrosschainTokenByChainId(params[0], params[8]).call();
            if (!tokenInfo[0]) {
...
async function setupCrosschainTokens(addresses) {
    try {
        ...
        const crosschainTokensInstance = new web3.eth.Contract(CrosschainTokens.abi, addresses.crosschainTokens);

        // 获取 FOUNDATION_MANAGER 角色
        const FOUNDATION_MANAGER = web3.utils.keccak256("FOUNDATION_MANAGER");
        const fManagers = await rolesInstance.methods.getRoleMemberArray(FOUNDATION_MANAGER).call();
        ...
        await proposeCrosschainToken(
            ...
            [
                addresses.sourceNetworkName === 'ganache1' ? 'UNIT' : 'WUNIT',
