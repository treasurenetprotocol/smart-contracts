...
## Deployment state (addresses)

- Deploy/upgrade scripts read historical addresses from `deployments/<network>.json`. When you run `npm run deploy:...` locally, the first step will create/update this file.
- In GitHub Actions, a step reads addresses from AWS Secrets Manager (`<network>/treasurenet-tnservices-dataprovider`) and writes the same structure to `deployments/<network>.json` before running scripts. New deployments append to this JSON and are later uploaded to S3/Secrets.
- If you do not have `deployments/<network>.json` locally, you can override addresses via `.env`:

```env
DAO_ADDRESS=0x...
MULSIG_ADDRESS=0x...
ROLES_ADDRESS=0x...
PARAMETER_INFO_ADDRESS=0x...
ORACLE_ADDRESS=0x...
GOVERNANCE_ADDRESS=0x...
TCASH_ADDRESS=0x...
TCASH_LOAN_ADDRESS=0x...
TCASH_AUCTION_ADDRESS=0x...
TAT_ADDRESS=0x...
```

Scripts prefer the JSON file first, then fall back to these `*_ADDRESS` environment variables. If a required address is missing, the script will throw.
