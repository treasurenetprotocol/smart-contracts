...
contract USTN is IERC20, Initializable, OwnableUpgradeable {
    event convert(uint256 _time, address _user, uint256 _unitValueTotal, uint256 _USTNAmount, string _type);
    ...
    function bidCost(address bider, uint256 amount) public onlyUSTNAuction returns (bool) {
    ...
    function bidBack(address bider, uint256 amount) public onlyUSTNAuction returns (bool) {
    ...
    function reduceTotalSupply(uint256 amount) public onlyUSTNFinance returns (bool) {
        _totalSupply -= amount;
    ...
    function addTotalSupply(uint256 amount) public onlyUSTNFinance returns (bool) {
        _totalSupply += amount;
    ...
    function addBalance(address add, uint256 amount) public onlyUSTNFinance returns (bool) {
        _balance[add] += amount;
    ...
    function reduceBalance(address add, uint256 amount) public onlyUSTNFinance returns (bool) {
        _balance[add] -= amount;
    ...
    function mint() public payable returns (bool result) {
        uint256 exchange_tokens = mintRate(msg.value);
    ...
    function mintBack(uint256 tokens) public payable returns (bool) {
        ...
        require(
            (_totalSupply - 5 * 10 ** 25) * getOSMValue(unit) / getOSMValue(ustn) >= tokens,
            "overflow the mintbak threshold"
        );
        uint256 exchange_tokens = mintBackRate(tokens);
        _balance[msg.sender] -= tokens;
        _totalSupply -= tokens;
