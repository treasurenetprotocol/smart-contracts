...
interface USTNFInterface {
    function auctionOver(address bider, uint256 tokens, uint256 a, uint256 b) external returns (bool);
}

contract USTNAuction is Initializable, OwnableUpgradeable {
    //_extractStatus 1: not extracted 2: extracted
    event auctionDetail(uint256 n, uint256 _time, address bider, uint256 _highValue, uint256 _extractStatus);
    ...
    event personalAuction(address _user, uint256 n, uint256 _sales);
    ...
    mapping(uint256 => address) private bider;
    mapping(uint256 => mapping(address => uint256)) private bidValue;
    mapping(uint256 => auctionbider[]) private bidUsers;
    mapping(uint256 => mapping(address => bool)) private _bider;
    ...
    //mortgage is the number of UNITs in the auction, debt is the starting auction price, and _debt is the interest
    ...
    function queryBider(uint256 n) public view returns (address, uint256) {
        return (bider[n], bidValue[n][bider[n]]);
    }

    function isbider(uint256 n, address pAddr) internal view returns (bool) {
        return _bider[n][pAddr];
    }

    //Bidding function, the n th auction item, the USTN of the bidAmount amount
    function bid(uint256 n, uint256 bidAmount) public returns (bool) {
    ...
    //Retrieve the USTN bid for the n th auction item, which can only be called when it is not currently the highest
    // price
    function bidWithdrawal(uint256 n) public returns (bool) {
    ...
    function getAuction(uint256 n) public returns (bool) {
        ...
        USTNFget.auctionOver(
            msg.sender, auctionList[n].sales, auctionList[n].debt, auctionList[n].startValue - auctionList[n].debt
        );
    ...
    function getAuctionBider(uint256 n) public view returns (auctionbider[] memory) {
        return bidUsers[n];
    }
    ...
    function upgradeState() public onlyFoundationManager returns (bool) {
        for (uint256 a = 0; a < auctionList.length; a++) {
