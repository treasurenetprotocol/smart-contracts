...
contract USTNAuction is Initializable, OwnableUpgradeable {
    ...
    modifier onlyUSTNFinance() {
        require(
            msg.sender == _ustnFinanceContract,
    ...
    function auctionStart(
        ...
        uint debt,
        uint _debt
    ) public onlyUSTNFinance returns (auctions memory) {
    ...
    function bid(uint n, uint bidAmount) public returns (bool) {
        require(
            ...
            "USTNAuction: auction is over"
        );
        require(
            auctionList[n].nowValue < (bidValue[n][msg.sender] + bidAmount),
            "USTNAuction: bid is not enough"
        );
        require(
            USTNget.bidCost(msg.sender, bidAmount),
            "USTNAuction: USTN is not enough"
        );
    ...
    function bidWithdrawal(uint n) public returns (bool) {
        ...
        require(
            USTNget.bidBack(msg.sender, bidValue[n][msg.sender]),
            "USTNAuction: bidBack failed"
        );
    ...
    function getAuction(uint n) public returns (bool) {
        ...
        require(
            auctionList[n].timeOver < block.timestamp,
            "USTNAuction: auction is not over"
        );
        require(
            bider[n] == msg.sender,
            "USTNAuction: you are not owner of bid"
        );
