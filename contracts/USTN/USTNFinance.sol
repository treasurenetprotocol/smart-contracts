...
interface USTNAInterface {
    ...
    function auctionStart(uint256 mortgage, uint256 debt, uint256 _debt) external returns (auctions memory);
}
...
contract USTNFinance is Initializable, OwnableUpgradeable {
    ...
    event loanRecord(
        ...
        uint256 _pledgeRate,
        uint256 _status,
        uint256 _operationStatus,
        uint256 _ustn,
        uint256 _unit,
        uint256 _unitValue,
        uint256 _pledgeRateBefore
    );
    //_type: deposit:1 withdrawal:2 interest:3
    event dwDetail(uint256 _time, uint256 _type, address _user, uint256 amount);
    ...
    uint256 private liquidationTime;
    uint256 constant multiple = 10 ** 18;
    ...
    function getOSMValue(bytes32 currencyName) internal view returns (uint256) {
    ...
    function _reserve(uint256 tokens) internal view returns (uint256) {
    ...
    function distributeDepositInterest() public onlyUser(FOUNDATION_MANAGER) returns (bool result) {
        ...
        uint256 point_total = 0;
        //calculate everyone deposit point
        for (uint256 c = 0; c < depositUsers.length; c++) {
            userCurrentTime = block.timestamp;
            uint256 o = (userCurrentTime - userPastTime[depositUsers[c]]) / liquidationTime;
        ...
        for (uint256 m = 0; m < depositUsers.length; m++) {
            userDepositBalances[depositUsers[m]] +=
                (loansTotalInterest * userDepositPoint[depositUsers[m]] / point_total);
            emit dwDetail(
                block.timestamp,
                3,
                depositUsers[m],
    ...
    function loansRate(uint256 tokens) public view returns (uint256) {
        uint256 exchange_amount =
            getOSMValue(unit) * tokens / getOSMValue(ustn) * getRatio.getPlatformConfig("loanPledgeRate") / 10000;
    ...
    function loans() public payable returns (bool) {
        ...
        uint256 loanAmount = loansRate(msg.value);
        uint256 _interest = loanAmount * getRatio.getPlatformConfig("loanInterestRate") / 10000;
        loanList memory acc;
        ...
        //emit loanRecord(msg.sender, loanAccount[msg.sender].length-1, msg.value, loanAmount + _interest,
        // getRatio.getUSTNLoanPledgeRate(), 1, 4, _interest, 0, 0, 0);
        return true;
    }

    //Add position operation, when the loan is lower than the warning line, the probability of being liquidated can be
    // alleviated by adding positions
    function addLoans(uint256 number) public payable returns (bool result) {
        require(loanAccount[msg.sender][number].liquidationState != 2, "liquidated");
        uint256 a = pledge(loanAccount[msg.sender][number].mortgage, loanAccount[msg.sender][number].debt);
        loanAccount[msg.sender][number].mortgage += msg.value;
        uint256 _rate = pledge(loanAccount[msg.sender][number].mortgage, loanAccount[msg.sender][number].debt);
    ...
    function loansBack(uint256 number, uint256 tokens) public payable returns (bool result) {
        require(USTNget.balanceOf(msg.sender) >= tokens);
        require(loanAccount[msg.sender][number].liquidationState != 2, "liquidated");
        uint256 backPercent = 0;
        ...
        uint256 _backUnit = loanAccount[msg.sender][number].mortgage * backPercent / multiple;
    ...
    function loanPossible() public view returns (uint256) {
        return bankLoanLimit;
    }

    function pledge(uint256 a, uint256 b) internal view returns (uint256) {
    ...
    function liquidation() public onlyUser("FoundationManager") returns (bool result) {
        ...
        for (uint256 a = 0; a < loanUsers.length; a++) {
            for (uint256 b = 0; b < loanAccount[loanUsers[a]].length; b++) {
                uint256 k = (userLoansCurrentTime - loanAccount[loanUsers[a]][b].timestamp) / liquidationTime;
                loanAccount[loanUsers[a]][b].timestamp += k * liquidationTime;
                uint256 interestBear = 0;
                if (loanAccount[loanUsers[a]][b].liquidationState != 2) {
                    while (k > 0) {
                        uint256 _interest =
                            loanAccount[loanUsers[a]][b].debt * getRatio.getPlatformConfig("loanInterestRate") / 10000;
                        loansTotalInterest += _interest;
                        interestMint += _interest;
                    ...
                    emit loanRecord(
                        ...
                        pledge(loanAccount[loanUsers[a]][b].mortgage, loanAccount[loanUsers[a]][b].debt),
                        1,
                        4,
                        interestBear,
                        0,
        ...
        for (uint256 m = 0; m < loanUsers.length; m++) {
            for (uint256 n = 0; n < loanAccount[loanUsers[m]].length; n++) {
                if ((loanAccount[loanUsers[m]][n]).liquidationState != 2) {
                    uint256 _x = pledge(loanAccount[loanUsers[m]][n].mortgage, loanAccount[loanUsers[m]][n].debt);
    ...
    function auctionOver(address bider, uint256 tokens, uint256 a, uint256 b) public returns (bool) {
