...
contract OilData is ProductionData {
        ...
        onlyWhenActive(_uniqueId)
    {
        ...
        emit TrustedProductionData(TREASURE_KIND, _uniqueId, _produceData.month, _produceData.amount);
    }
        ...
        onlyWhenActive(_uniqueId)
    {
        ...
        uint256 discount = _parameterInfo.getPriceDiscountConfig(thisProducer.API, thisProducer.sulphur);

        // Market value
        /* Decimal design: Multiply the 'amount' parameter by 10000 for the input, and multiply the unit price by 10000
        for the input */
        uint256 price =
            ((_produceData.amount * _getAssetValue(_produceData.date) * discount) * 1e18) / 10000 / 10000 / 10000;
        ...
        for (uint256 i = 0; i < _uploadedProduceData[_produceData.month].length; i++) {
            if (_uploadedProduceData[_produceData.month][i].uniqueId == _uniqueId) {
                _exist = true;
                require(
                    _uploadedProduceData[_produceData.month][i].status == ProduceDataStatus.UNAUDITED,
                    "production data that has been audited."
                );
                _uploadedProduceData[_produceData.month][i].amount =
                    _uploadedProduceData[_produceData.month][i].amount + _produceData.amount;
                _uploadedProduceData[_produceData.month][i].price =
                    _uploadedProduceData[_produceData.month][i].price + price;
                break;
            }
        }
        if (!_exist) {
        ...
        emit ProducerProductionData(
            TREASURE_KIND, _uniqueId, _produceData.month, _produceData.date, _produceData.amount, price
        );
    }
    ...
    function _clearing(bytes32 _uniqueId, uint256 _month) internal override returns (bool) {
        ...
        require(uploaded.status == ProduceDataStatus.UNAUDITED, "this product data already audited");
        ...
        require(
            _rewardByShare(_uniqueId, uploaded.price) == uploaded.price, "total minted must equal to trusted.amount"
        );
