...
module.exports = async function (deployer, network, accounts) {
  try {
    ...
    fs.appendFileSync(
      "contracts.txt",
      `const CROSSCHAIN_TOKENS_ADDRESS='${crosschainTokens.address}'\n`
    );

    // 部署CrosschainBridge跨链桥合约
    const crosschainBridge = await deployProxy(
      CrosschainBridge,
    ...
    fs.appendFileSync(
      "contracts.txt",
      `const CROSSCHAIN_BRIDGE_ADDRESS='${crosschainBridge.address}'\n`
    );

    // 更新CrosschainTokens中的MulSig地址
    const crosschainTokensInstance = await CrosschainTokens.at(
