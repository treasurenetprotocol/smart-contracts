...
/**
 * @title TreasureOilTest
 * @dev Test contract for oil producer functionality
 * Follows AAA pattern: Arrange, Act, Assert
 */
contract TreasureOilTest is BaseTest {
    using Strings for uint256;
    
    // Contract instances
    OilProducer public oilProducer;
    OilData public oilData;
    TAT public tat;
    
    // Test constants
    bytes32 constant WELL_UNIQUE_ID = bytes32(0x4872484e4579694e575a65745956524879303873690000000000000000000003);
    string constant WELL_NICKNAME = "Treasure-Oil";
    string constant MINTING_ACCOUNT_STRING = "0xe1aabd4ed39af0f1e023b6f1bca4e01237418aff";
    uint256 constant WELL_API = 3000;    // OIL requires non-zero API
    uint256 constant WELL_SULPHUR = 480; // OIL requires non-zero sulphur
    ...
    function setUp() public override {
        ...
        oilProducer.initialize(
            ...
            "OIL",                // assetType
            address(oilData),     // productionDataContract
            dappNames,            // dappNames
            payees                // payees
        );
        
        // Initialize OilData contract
        oilData.initialize(
            "OIL",                // _treasureKind
            address(0),           // _oracleContract
            DUMMY_ROLE,           // _rolesContract
            address(0),           // _parameterInfoContract
            address(oilProducer), // _producerContract
    ...
    function testAddProducerAndModifyStatus() public {
        ...
        oilProducer.addProducer(WELL_UNIQUE_ID, producer);
        vm.stopPrank();
        ...
        oilProducer.setProducerStatus(WELL_UNIQUE_ID, IProducer.ProducerStatus.Active);
        vm.stopPrank();
        
        // === ASSERT ===
        // Get producer data and verify
        (IProducer.ProducerStatus status, IProducer.ProducerCore memory result) = oilProducer.getProducer(WELL_UNIQUE_ID);
