...
describe('TokenLocker', function () {
  it('manages managers list and prevents duplicates', async function () {
  ...
  it('validates plan creation and tracks available balances', async function () {
    const { locker, accounts } = await loadFixture(deployLockerFixture);
  ...
  it('creates locked records and respects plan limits and claim method', async function () {
    const { locker, accounts } = await loadFixture(deployLockerFixture);
    const planId = bytes('PLAN1');
  ...
  it('lets beneficiaries and managers claim unlocked tokens respecting claimMethod', async function () {
    const { locker, accounts } = await loadFixture(deployLockerFixture);
    ...
    await expect(() =>
      locker.connect(accounts.user1).claimToken(accounts.user1.address)
    ...
    await expect(() =>
      locker.connect(accounts.manager).claimToken(accounts.user2.address)
    ).to.changeEtherBalances(
  ...
  it('deletes locked records and returns funds to availability', async function () {
    const { locker, accounts } = await loadFixture(deployLockerFixture);
    const planId = bytes('PLAN1');
    const lockedId = bytes('LOCK1');
  ...
  it('deactivates plan and reclaims remaining and active record amounts', async function () {
    const { locker, accounts } = await loadFixture(deployLockerFixture);
    const planId = bytes('PLAN1');
