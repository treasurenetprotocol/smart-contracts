...
contract DeployTreasureNet is Script {
    ...
    function run() external {
        ...
        dao = DAO(deployProxy(address(daoImpl), abi.encodeWithSelector(DAO.initialize.selector, "DAO", 2, 10)));
        ...
        mulSig = MulSig(deployProxy(address(mulSigImpl), ""));
        console.log("MulSig deployed at:", address(mulSig));

        roles = Roles(deployProxy(address(rolesImpl), ""));
        ...
        // Deploy TCash
        tcash = TCash(deployProxy(address(tcashImpl), abi.encodeWithSelector(TCash.initialize.selector)));
        ...
        // Initialize MulSig
        mulSig.initialize(
            address(dao), address(governance), address(roles), address(parameterInfo), address(crosschainTokens), 2
        );
        ...
        crosschainBridge = CrosschainBridge(
            deployProxy(
                ...
                abi.encodeWithSelector(CrosschainBridge.initialize.selector, address(crosschainTokens), address(roles))
            )
        );
        ...
        airDrop = AirDrop(deployProxy(address(airDropImpl), ""));
